...
class GeneralizedPareto(BaseDistribution):
    ...
    def _pdf(self, x):
        ...
        return genpareto.pdf(x, c, mu=mu, scale=scale)

    def _cdf(self, x):
        c = self._bc_params["c"]
        scale = self._bc_params["scale"]
        mu = self._bc_params["mu"]
        return genpareto.cdf(x, c, mu=mu, scale=scale)

    def _ppf(self, p):
        c = self._bc_params["c"]
        scale = self._bc_params["scale"]
        mu = self._bc_params["mu"]
        return genpareto.ppf(p, c, mu=mu, scale=scale)

    def _mean(self):
        c = self._bc_params["c"]
        scale = self._bc_params["scale"]
        mu = self._bc_params["mu"]
        return genpareto.mean(c, mu=mu, scale=scale)
    ...
    def _var(self):
        ...
        v = genpareto.var(c, mu=mu, scale=scale)
