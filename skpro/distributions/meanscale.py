...
class MeanScale(BaseDistribution):
    ...
    If :math:`X` is distributed according to ``d``, then
    ...
    This distribution also parameterizes the natural mean/scale family of distirbutions
    generated by the component distribution ``d``,
    parameterized a mean offset :math:`\mu`, represented by ``mu``, and
    scale factor :math:`s`, represented by ``sigma``.
    ...
    mu : float or array of float (1D or 2D)
        additive offset of the component distribution
    sigma : float or array of float (1D or 2D), must be positive
        multiplicative scaling of the component distribution,
        must be positive, i.e. ``sigma > 0``
    ...
    >>> d_scalar.energy()  # E|X-Y| = sigma * E|Y-Z| (where Y, Z ~ d)
    """
