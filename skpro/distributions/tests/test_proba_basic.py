...
def test_energy_nan_propagation_empirical():
    """Test that Empirical.energy returns NaN for all-NaN sample rows.

    Regression test: np.sum on a pandas DataFrame used skipna=True by default,
    ...
    assert np.isnan(dist_nan.energy(y_true).values).all(), (
        "energy(y_true) should be NaN for all-NaN Empirical samples"
    )
    assert np.isnan(dist_nan.energy().values).all(), (
        "energy() should be NaN for all-NaN Empirical samples"
    )
