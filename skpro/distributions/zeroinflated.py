...
class ZeroInflated(BaseDistribution):
    ...
    Combines a Bernoulli gate for structural zeros with a with a standard count
    distribution defined on non-negative integers for the outcomes, allowing
    zeros to arise from both components.
    ...
    count distribution.
    :math:f(x \mid x \ge 0) is the probability mass function of the count
    distribution defined on the set of non-negative values

    Parameters
    ----------
    p : ArrayLike
        The probability of the count process being active
        Must be in [0, 1].
