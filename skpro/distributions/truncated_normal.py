...
class TruncatedNormal(_ScipyAdapter):
    ...
    def _energy_self(self):
        ...
        it = np.nditer([mu_b, sigma_b, l_b, r_b, result], flags=["multi_index"], op_flags=[["readonly"]]*4 + [["writeonly"]])
        for m, s, l, r, out in it:
            a = (l.item() - m.item()) / s.item()
            b = (r.item() - m.item()) / s.item()
            def cdf(x):
                return truncnorm.cdf(x, a, b, loc=m.item(), scale=s.item())
            def integrand(x):
                F = cdf(x)
                return 2 * F * (1 - F)
    ...
    def _energy_x(self, x):
        ...
        r_trunc = np.asarray(self._bc_params["r_trunc"])
        x = np.asarray(x)
        mu_b, sigma_b, l_b, r_b, x_b = np.broadcast_arrays(mu, sigma, l_trunc, r_trunc, x)
        result = np.empty_like(mu_b, dtype=float)
        it = np.nditer([mu_b, sigma_b, l_b, r_b, x_b, result], flags=["multi_index"], op_flags=[["readonly"]]*5 + [["writeonly"]])
        for m, s, l, r, x0, out in it:
            a = (l.item() - m.item()) / s.item()
            b = (r.item() - m.item()) / s.item()
            def integrand(t):
                return np.abs(t - x0.item()) * truncnorm.pdf(t, a, b, loc=m.item(), scale=s.item())
            val, _ = quad(integrand, l.item(), r.item(), limit=200)
            out[...] = val
        # Always flatten to 1D of length n_rows for DataFrame compatibility
        n_rows = 1 if self.index is None else len(self.index)
