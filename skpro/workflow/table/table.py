...
class Table:
    ...
    def __init__(self, tasks=[], modifiers=[]):
        self.tasks = tasks
    ...
    def render(self, models, verbose=0, debug=False):
        ...
        for model in models:
            if not issubclass(model.__class__, Model):
                raise Exception('Model has to be subclass instance skpro.workflow.Model')

            if verbose > 1:
                print('Evaluating model %i/%i: %s' % (i+1, len(models), model.description()))
            row = []
            for controller, view in self.tasks:
                controller_id = controller.identifier()
                if not debug or controller.description() == 'Info':
                ...
                if verbose > 2:
                    print('Running ' + controller.description())
                    print(model[controller_id])

                if i == 0:
                    headers.append(controller.description())
        ...
        for raw in raw_tbl:
            ...
            for cell in raw:
                if 'view' in cell:
                    row.append(cell['view'].parse(cell['data']))
            tbl.append(row)
