...
class ProbabilisticStackingRegressor(BaseProbaRegressor):
    ...
    This class is inspired by modular, composable stacking/meta-learning frameworks that produce
    calibrated probabilistic or quantile predictions, as described in:

    - Dudek, G. (2024). Stacking for Probabilistic Short-term Load Forecasting. arXiv:2406.10718.
      https://arxiv.org/abs/2406.10718
    - Large, J., et al. (2019). A probabilistic classifier ensemble weighting scheme based on cross-validated accuracy estimates (CAWPE). Machine Learning. https://pmc.ncbi.nlm.nih.gov/articles/PMC6790343/

    For future extensibility, consider allowing meta-learners (e.g., quantile regression, QRF, CAWPE weighting)
    and supporting arbitrary pipeline branches as base estimators.
    ...
    def __init__(self, estimators, weights=None, meta_learner=None, use_meta_proba=True):
        """
        Parameters
        ----------
        estimators : list of (str, BaseProbaRegressor)
        ...
        use_meta_proba : bool, default True
            If True and meta_learner supports predict_proba, use it for probabilistic output.
        """
    ...
    def _predict_proba(self, X):
        ...
        if self.meta_learner_ is not None:
            import numpy as np
            base_preds = [est.predict(X).values.flatten() for _, est in self.fitted_estimators_]
    ...
    def get_test_params(cls, parameter_set="default"):
        ...
        return {"estimators": [("est1", est1), ("est2", est2)]}

class ProbabilisticBoostingRegressor(BaseProbaRegressor):
    """Residual-based probabilistic boosting ensemble for skpro.

    Trains base probabilistic regressors sequentially, each on the residuals of the previous prediction (y_true - y_pred_mean).
    Final prediction is a weighted mixture of all base predictions, as a true boosting ensemble for probabilistic regression.

    This class is inspired by modular, composable probabilistic boosting frameworks, including:

    - Mendonça, A., et al. (2022). ProBoost: a Boosting Method for Probabilistic Classifiers. 
    https://arxiv.org/abs/2209.01611
    - Sprangers, P., Schelter, S., & de Rijke, M. (2021). Probabilistic Gradient Boosting Machines (PGBM) for Large-Scale Probabilistic Regression. 
    https://arxiv.org/abs/2106.01682
    - Tu, Z. (2005). Probabilistic Boosting-Tree: Learning Discriminative Models for Classification, Recognition, and Clustering. ICCV 2005. 
    ...
    """
    def __init__(self, base_estimator, n_estimators=10, learning_rate=1.0, uncertainty_weighting=None, calibrator=None):
        """
        Parameters
        ----------
        base_estimator : BaseProbaRegressor
        ...
        uncertainty_weighting : callable or None
            If provided, a function that takes (y_true, y_pred, round_idx) and returns a weight for this round.
            Inspired by ProBoost and PGBM for uncertainty-aware weighting.
        calibrator : object or None
            If provided, should have fit(y_true, y_pred) and predict_proba(y_pred) for post-hoc calibration.
        """
    ...
    def _fit(self, X, y, C=None):
        ...
        for i in range(self.n_estimators):
            ...
            if hasattr(y_pred, 'to_frame'):
                y_pred = y_pred if isinstance(y_pred, type(y)) else y_pred.to_frame(index=y.index, name=y.columns[0] if hasattr(y, 'columns') else None)
            residual = y - y_pred
    ...
    def get_test_params(cls, parameter_set="default"):
        ...
        return {"base_estimator": base, "n_estimators": 3}
