...
Bayesian Linear Regression defined with normal priors for slopes and intercept
and half-normal prior for noise; coded on pymc backend.
"""
...
class BayesianLinearRegressor(BaseProbaRegressor):
    ...
    noise_alpha : float, optional (default=1)
        Alpha parameter for the inverse gamma prior for the noise variance.
    noise_beta : float, optional (default=1)
        Beta parameter for the inverse gamma prior for the noise variance.
    chains : int, optional (default=2)
        Number of MCMC chains to run.
    draws : int, optional (default=2000)
        Number of draws sampled from each chain; applied to all sampling steps.
    ...
    def __init__(
        ...
        assert self.noise_alpha > 0, "noise_alpha must be positive"
        assert self.noise_beta > 0, "noise_beta must be positive"
        assert (
            isinstance(self.chains, int) and self.chains > 0
        ), "chains must be a positive integer"
        assert (
            isinstance(self.draws, int) and self.draws > 0
    ...
    def _fit(self, X, y):
        ...
        with pm.Model(coords={"obs_id": X.index, "pred_id": X.columns}) as self.model:
            ...
            self.intercept = pm.Normal(
                "intercept", mu=self.intercept_mu, sigma=self.intercept_sigma
            )
            self.slopes = pm.Normal(
                "slopes",
                mu=self.slopes_mu,
                sigma=self.slopes_sigma,
    ...
    def get_prior(self, return_type="xarray"):
        ...
        assert return_type in [
            ...
            "numpy",
            "dataframe",
        ], "return_type must be one of 'xarray', 'numpy' or 'dataframe"

        with self.model:
            # if we've previously used the model for prediction,
            # we need to reset the reference of 'X' to the training data
    ...
    def _predict(self, X):
        ...
        assert X.columns.equals(
            self._X_cols
        ), f"The columns of X must be the same as the columns of the training data: {self._X_cols}"
        ...
        with self.model:
            ...
            self.trace.extend(
                pm.sample_posterior_predictive(
                    self.trace, random_seed=42, predictions=True
                )
            )
